...
async function main() {
	try {
		...
		try {
            ...
            console.log("=====Read Asset Price from shared collection =>",assetCollection)
            result = await contractOrg2.evaluateTransaction('ReadAssetPrice', assetID,assetCollection);//should change how AgreeToTransfer is implemented
            console.log(`<-- result: ${prettyJSONString(result.toString())}`);
            await sleep(3000);
            ...
            console.log("=====GetAssetBidPrice from org 2")
            result = await contractOrg2.evaluateTransaction('GetAssetBidPrice', assetID);//should change how AgreeToTransfer is implemented
            console.log(`<-- result: ${prettyJSONString(result.toString())}`);
            await sleep(3000);
            ...
            console.log('\n~~~~~~~~~~~~~~~ We have to delete previous bid request ~~~~~~~~~~~~~~~~');
            transaction = contractOrg2.createTransaction('DeleteBidRequest');
            transaction.setEndorsingOrganizations(mspOrg2);
            transaction.submit(assetID);
            await sleep(3000);
            ...
            try {
				...
				await transaction.submit(assetID);
				console.log(`*** Result: committed, Org2 has agreed to sell asset ${assetID} for 150`);
			} catch (sellError) {
				console.log(`${RED}*** Failed: AgreeToSell - ${sellError}${RESET}`);
			}
            await sleep(2000);
			...
			try {
				...
				// });
				await transaction.submit(assetID);
				console.log(`*** Result: committed, Org3 has agreed to buy asset ${assetID} for 150`);
			} catch (buyError) {
				console.log(`${RED}*** Failed: AgreeToBuy - ${buyError}${RESET}`);
			}
            await sleep(3000);

            console.log("=====Read Asset Price from org3 shared collection =>",sharedCollectionOrg2Org3)
            result = await contractOrg3.evaluateTransaction('ReadAssetPrice', assetID,sharedCollectionOrg2Org3);//should change how AgreeToTransfer is implemented
            console.log(`<-- result: ${prettyJSONString(result.toString())}`);
            await sleep(3000);


			try {
                console.log("=====GetAssetBidPrice from org 2,shoudl show error cause bid from org 2 was deleted from implicit collection")
                result = await contractOrg2.evaluateTransaction('GetAssetBidPrice', assetID);//should change how AgreeToTransfer is implemented
                console.log(`<-- result: ${prettyJSONString(result.toString())}`);
                await sleep(3000);
			} catch (buyError) {
				console.log(`${RED}*** Failed: GetAssetBidPrice - ${buyError}${RESET}`);
			}

            
            console.log("=====GetAssetBidPrice from org 3")
            result = await contractOrg3.evaluateTransaction('GetAssetBidPrice', assetID);//should change how AgreeToTransfer is implemented
            console.log(`<-- result: ${prettyJSONString(result.toString())}`);
            await sleep(3000);
